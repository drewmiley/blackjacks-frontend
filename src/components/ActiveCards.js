          ...
          <div>
            {isNominatedSuit ?
                (
                    <div>
                        <div className={styles.infoText}>
                            {!isInitialPileCard ? `Nominated suit is ${activeCards.suit}` : `Initial card is ${JACKS_TWOS_EIGHTS_NOMINATION_VALUE} of ${lastCardsPlayed[lastCardsPlayed.length -1].suit}. Free choice`}
                        </div>
                        <Hand cards={[{ rank: 11, suit }]} hidden={false} style={HAND_STYLE} />
                    </div>
                ) :
                <div><Hand cards={[card]} hidden={false} style={HAND_STYLE} /></div>
            }
